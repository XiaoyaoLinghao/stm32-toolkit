...
- [ ] **Step 1: Freeze preconditions.** Record CodeHead/status, design/plan SHAs, old candidate/runtime identity, exact backend wheel hashes, Python 3.12.10, and empty campaign scratch. If an already-recorded exact backend wheel is absent, stop and classify before acquisition. After the classified absence recorded on 2026-08-25, execute only the design's one-time official-PyPI direct-artifact recovery into a run-scoped scratch directory: no resolver/index/dependencies/redirects, require the frozen filename/size/SHA-256 tuple for both files, preserve acquisition evidence, then resume with the identical 66-wheel closed set. Any mismatch stops before build/runtime/project/hardware mutation.
