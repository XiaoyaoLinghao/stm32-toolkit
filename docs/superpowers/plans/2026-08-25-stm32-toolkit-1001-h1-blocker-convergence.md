...
- [ ] **Step 1b: Converge the revealed three-path ARMCC syntax correction before retrying Step 2.** From clean portability commit `b83b404c8da6993658586fa1b553d715f95993c1`, create a scratch-only proposal for exactly `Common/common.c`, `MALLOC/malloc.c`, and `USER/usart1/usart1.c` under design Section 5.4. Compile the three units and inspect the CMSIS-intrinsic object instructions and allocator address/range facts. In a disposable exact project worktree, require public inspect PASS and two identical public convert dry-runs with zero blockers. Preserve one canonical three-path `git diff --binary --full-index`, closed before/after hashes, and self-contained evidence; remove scratch. Sol independently verifies and writes an exact digest authorization. Only then apply the identical diff to the campaign project, commit locally, and record authorization consumption. No conversion apply, configuration, build, hardware, or remote action is allowed before that commit.
