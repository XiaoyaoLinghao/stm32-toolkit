...
- [ ] **Step 4: Implement the minimal evidence helper.** Use case-insensitive basenames and a bounded canonical peripheral-name regex; require both source classes before appending the one stable evidence item. Do not inspect directory/project names or add a public option.
...
- [ ] **Step 1: Freeze preconditions.** Record CodeHead/status, design/plan SHAs, old candidate/runtime identity, exact backend wheel hashes, Python 3.12.10, and empty campaign scratch. No network acquisition is permitted unless an already-recorded exact wheel is absent; then stop and classify.
