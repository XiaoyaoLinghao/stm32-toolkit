...
- [ ] **Step 1b: Converge the revealed four-path ARMCC/GCC source correction before retrying Step 2.** Treat the retained three-path proposal as diagnosis-only `PROJECT_INPUT/BLOCKED`; it cannot be authorized or applied. From clean portability commit `b83b404c8da6993658586fa1b553d715f95993c1`, create a replacement scratch-only proposal for exactly `Common/common.c`, `MALLOC/malloc.c`, `USER/usart1/usart1.c`, and `Main/string.h` under design Section 5.4. Preserve the complete legacy ARMCC header body byte-for-byte behind an outer non-GNU branch and make only the GNU branch forward with `#include_next` to the pinned CubeCLT newlib header. Compile the three directly corrected units and inspect the CMSIS-intrinsic object instructions and allocator address/range facts; then compile every C source selected by `Project/LWIP.gcc.uvprojx` in isolation with the frozen GCC/include/define contract and prove all selected `string.h` consumers traverse the forwarding header to CubeCLT newlib. Any compile error stops with classified evidence; warnings are recorded. In a disposable exact project worktree, require public inspect PASS and two identical public convert dry-runs with zero blockers. Preserve one canonical four-path `git diff --binary --full-index`, closed before/after hashes, dependency-resolution evidence, and a self-contained replacement proposal; remove scratch. Sol independently verifies and writes an exact digest authorization. Only then apply the identical diff to the campaign project, commit locally, and record authorization consumption. No conversion apply, configuration, build, hardware, or remote action is allowed before that commit.
