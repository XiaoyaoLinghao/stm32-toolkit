...
Compatibility clarification: Bootstrap/Repair finalization requires the exact release pin. Read-only Check retains its existing supported installed-module range `>=0.45.1,<0.46` and additionally requires the final launcher version to match that already validated module version. It must not silently tighten an existing Check contract to one patch release or accept module/launcher disagreement.
