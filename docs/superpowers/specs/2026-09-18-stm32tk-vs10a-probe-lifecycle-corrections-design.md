...
Terminality must come from the owning backend, not from guessing that every address/register error ends an attachment. The production worker already owns `_closed` and actual child liveness. A minimal internal read-only terminal-state property in `probe/worker.py` may expose their disjunction for service cache invalidation; do not alter the existing `is_alive` process-death meaning used by cleanup, or expose a new public RPC/schema. This property must also invalidate a closed but not yet successfully terminated worker. Cache lookup must reject known-terminal state even if the error originated in a private handoff-metadata call; stop/close retain their existing explicit invalidation and cleanup order. In-process recoverable-error seams without terminal evidence retain their healthy attachment. Tests distinguish terminal worker errors from recoverable backend errors rather than weakening either contract.
