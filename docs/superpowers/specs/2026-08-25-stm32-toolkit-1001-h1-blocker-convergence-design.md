...
The guarded apply validator must recognize exactly that two-part contract rather than require one global blocker sort. Re-derive the expected scoped suffix from the plan's bound inspection using the same pure planner helper. The prefix must retain the existing global sort/path/type validation; the suffix must equal the re-derived scoped records byte-for-byte and in authored order. An empty blocker path is valid only inside an exact re-derived group-scoped suffix record; arbitrary empty paths and malformed/reordered/removed/inserted suffix records remain `MIGRATION_PLAN_INVALID`. Plan ID, inspection digest, fresh-plan equality, Git/root/input/patch validation, and all no-write guarantees remain unchanged. A structurally valid plan with any scoped blocker reaches the ordinary `MIGRATION_BLOCKED` result before the first write.

TDD covers include-only group options, define-only file options, mixed options producing one blocker rather than one per field, duplicate-owner group records remaining distinct/in authored order, long-owner evidence bounded to 200 codepoints, unchanged non-scoped deduplication, unchanged support for target defines/includes, and unchanged refusal for target/group/file misc controls. Apply regressions cover a group-only empty-path blocker and a globally out-of-order-looking existing-prefix plus file-scoped suffix reaching `MIGRATION_BLOCKED`, along with forged suffix/path/order rejection and zero writes. CLI/MCP/workflow parity must expose the same plan blockers. No schema, public command, project-specific path, option-flattening rule, or build backend is added.
...
Preserve the map-fix runtime identity and no-process proof, retire only the exact campaign-managed `data/runtime`, and fresh-bootstrap the accepted replacement; same-version upgrade/repair remains forbidden. On the unchanged clean project commit `b83b404c8da6993658586fa1b553d715f95993c1`, public inspect must retain the real baseline/SPL facts and expose the two scoped group options. Public convert dry-run must now return exactly twelve blockers: the retained `Startup_config/startup_stm32f429_439xx.s` assembly blocker proven by Task 3b, the prior four inline-assembly, four absolute-placement, and one no-semihosting blockers, plus two `ARMCC_OPTION_UNSUPPORTED` group blockers with evidence `group:Main` and `group:USER`. The earlier eleven-blocker wording omitted the already-proven assembly finding from its arithmetic; that was a specification/report defect, not product drift. This corrected blocker set is the required proof of the scoped-option correction. Only then may the five-path project proposal resume.
