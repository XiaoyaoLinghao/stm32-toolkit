...
## Normal Target 04: physical PASS after user power cycle

The user reported normal D4 flashing after reconnecting the board, then explicitly authorized complete normal Target verification. Fresh session `vs10a-t9t10-p2-20260911-04` ran one normal prepare and one execute against unchanged deployment `5f036363383e6c85cc87426da1b4a1c20dfe7acc` and P2. Both exited 0; no under-reset mode or automatic retry was used. The same original-exception capture was retained in-process for execute but no error was raised; product files were unchanged.

Authoritative physical TestRun `target-v2-c3ff80b549561c68d3a5961072d728ba` passed `d4-heartbeat` (1 passed, 0 failed/error/timeout), mailbox duration 2200 ms. Evidence ID: `6f5266ab104c728207c7d8c097d8ee48fa442fbb59aeaaa146fc95bab7df6d59`; test-manifest SHA256: `c72b05a7fd3412e970124da512b4a092227826c232d91520a031dd7555d57c0e`. Public `test show` independently reloaded the authoritative physical result. The current flash receipt records success with 54,896 verified bytes and exactly matches the TestRun's workspace, session, build, ELF, input snapshot, Git and target identities; probe hash matches the fixed selector.

The user confirmed D4 was still flashing after the test. The lease is released and runtime process count is zero. P2 source/ELF and historical -12 100ms and attempt 7 evidence hashes are unchanged. Evidence and consumed authorization are retained at `D:\codex-tmp\t9t10-p2-20260911-04`; the canonical flash receipt now belongs to this fresh session and supersedes the prior receipt for subsequent current-firmware operations.

**P2 normal Target physical verification is complete.** This confirms the repaired environment now permits the complete normal path; it does not retrospectively prove the precise earliest exception in attempt 01. Remaining work includes P2 required observations, actual T9 IDE handoff/reacquisition, T10 P3/P4 Diagnostic and FixVerification, and final lineage/diff acceptance. T9/T10/VS10-A are not marked complete.

## Observation 05: acceptance-card precondition corrected

Under “继续推进验收工作”, the primary invoked one normal Fault workflow in the accepted P2 session. It correctly refused with `FAULT_TARGET_NOT_HALTED`, details `state=running`. This is a REPORT/acceptance-plan error: the card incorrectly required full Fault analysis on a running target. `debug/fault.py:228-244` requires already-halted core register access; `analyze_fault:514-558` reads initial and final registers and requires stability without target control. The existing 2026-09-10 protocol state-alignment plan already records this prerequisite. No product rule should be weakened and this does not indicate a new hardware fault.

The workflow stopped; no later sample, GPIO read or T9 handoff ran. No Fault report or evidence of “no active Fault” was produced. Target running state is the only new target-state fact. The lease is released, runtime process count is zero, and the accepted P2 flash receipt is unchanged. Evidence: `D:\codex-tmp\t9t10-p2-observe-20260911-05\result.json` and `fault.json`.

The serial card now separates bounded running-state testtime/GPIO observations from full halted-state Fault acceptance. Full Fault remains pending until its explicit control authority, probe ownership and an existing entry that preserves the halted prerequisite are established; it must not compete with the IDE during handoff. Existing P2 physical PASS and -12/attempt7 evidence remain valid. T9/T10/VS10-A remain incomplete.
