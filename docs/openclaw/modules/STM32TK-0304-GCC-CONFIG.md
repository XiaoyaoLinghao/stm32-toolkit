...
- `.vscode/tasks.json` contains the deterministic Build Debug and Build Release process tasks. Every task command is `stm32-toolkit`; args are arrays using `${workspaceFolder}`. No generated task silently starts or ends an external debug handoff.
- `.vscode/launch.json` contains `{"version":"0.2.0","configurations":[]}` for both build-only and hardware-capable projects. It is managed project input and participates in the input snapshot, so a live handoff configuration must never be written back to it. After a successful authorized handoff, the caller uses a run-owned `.code-workspace` outside the project input tree, with `folders` pointing only to the original project, and wraps the returned Cortex-Debug PyOCD attach fragment in workspace-level `launch.configurations` with its `name` and `type`, using `targetId`, raw `boardId`, public `serialNumber`, and executable. The caller explicitly ends the same ticket after IDE detach; no automatic IDE generation or launch framework is added. If debug backend/target is absent or backend is not `pyocd`, return `GENERATION_MODEL_INVALID`; do not guess.
