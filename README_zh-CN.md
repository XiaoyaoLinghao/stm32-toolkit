...
本仓库包含本地 **0.9.0 VS09-A candidate**，尚未 push、打 tag 或 release。发布契约是
CPython `>=3.12,<3.13`；托管解释器只能使用 `DATA_ROOT/runtime/0.9.0/Scripts/python.exe`。
MCP 绝不回退到系统解释器。setup helper 的 CHECK 模式只读；Bootstrap 和 Repair 必须得到
明确授权，在本地 staging、校验 Toolkit 与 Monitor 包后才可提升。
...
licenses、compatibility 和 troubleshooting。本地 VS09-A candidate 不将这些内容声称为已
完成的 release 证据；硬件、远程、PR、merge、tag 和 release 操作同样不在本 candidate 内。
